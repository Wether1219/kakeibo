      ...
      <table className="min-w-full border border-gray-200 text-sm">
        <thead>
          <tr className="bg-gray-50">
            <th rowSpan={2} className="px-3 py-2 text-left border-b border-gray-200 align-bottom">
              費目
            </th>
            {weeks.map((w) => (
              <th key={w} colSpan={2} className="px-3 py-2 text-center border-b border-gray-200">
                第{w}週
              </th>
            ))}
          </tr>
          <tr className="bg-gray-50">
            {weeks.map((w) => (
              <Fragment key={w}>
                <th className="px-3 py-1 text-right border-b border-gray-200 font-normal text-xs text-gray-500">
                  予算
                </th>
                <th className="px-3 py-1 text-right border-b border-gray-200 font-normal text-xs text-gray-500">
                  実績(差額)
        ...
        <tbody>
                ...
                return (
                  <Fragment key={w}>
                    ...
                    <td className="px-3 py-2 text-right whitespace-nowrap">
                      ...
                      <span className={`ml-1 text-xs ${diff < 0 ? 'text-red-600' : 'text-gray-400'}`}>
                        ({diff >= 0 ? '+' : ''}
                        {diff.toLocaleString()})
                      </span>
                    </td>
          ...
          {categories.length === 0 && (
            <tr>
              <td className="px-3 py-4 text-gray-400" colSpan={weeks.length * 2 + 1}>
