...
function CategoryRows({
  ...
  return (
    <>
      {categories.map((c) => (
        <div key={c.categoryId} className="flex justify-between pl-3 text-xs text-gray-400">
          <dt>
            {c.icon ?? ''}
            {c.name}
          </dt>
  ...
  return (
    <section className="space-y-2">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-gray-600">月平均の収支（毎月発生するもの）</h2>
        <button
          type="button"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {members.map((m) => (
          <div key={m.userId} className="rounded border border-gray-200 bg-white p-4 shadow-sm">
            <h3 className="text-sm font-bold text-gray-500 mb-3">{m.displayName}</h3>
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <dt className="text-gray-500">固定費</dt>
              ...
              <div className="flex justify-between">
                <dt className="text-gray-500">変動費</dt>
              ...
              <div className="flex justify-between border-t border-gray-100 pt-2">
                ...
                <dd className={`font-bold ${m.monthlyAverage.remainingSaving < 0 ? 'text-red-600' : ''}`}>
            ...
            <div className="mt-3 pt-3 border-t border-gray-100 flex justify-between text-sm bg-amber-50 -mx-4 -mb-4 px-4 py-2 rounded-b">
              <dt className="text-gray-500">💰賞与（年間合計÷2）</dt>
